...
public class DefaultJeepOrderDao implements JeepOrderDao {
  ...
  public Order saveOrder(Customer customer, Jeep jeep, Color color, Engine engine, Tire tire,
   ...
   KeyHolder keyHolder = new GeneratedKeyHolder(); //receives the primary key 
   jdbcTemplate.update(params.sql, params.source, keyHolder); 
   
   Long orderPK = keyHolder.getKey().longValue(); //extracts the order primary key
   saveOptions(options, orderPK);
   
   //@formatter:off
       //using the data from the Order class, passing in the method params
